...
# scrcpy --v4l2-sink=/dev/video9 -m 640 --max-fps=60 --window-x=100 --always-on-top # --lock-video-orientation=2
# robocon_wsから実行される
...
def main():
    with ThreadPoolExecutor(max_workers=2) as executor: # max_workers=3
        executor.submit(pub)
        executor.submit(detect)
        # executor.submit(run_command)

def run_command():
    # command = "sudo modprobe v4l2loopback video_nr=9"
    # password = "robocon471\n".encode()
    # subprocess.run(command.split(" "), input=password)

    command = "scrcpy --v4l2-sink=/dev/video9 -m 640 --max-fps=60 --window-x=100 --always-on-top"
...
def detect():
    ...
    margin_top = [0,5,8,10] # 上からの余白。認識しても無視する。何もないとき,ブルーベリー,ぶどう,ミックス
    while True:
        if count == max_count:
            timer.stop()
