            ...
            if fit == True:
                ...
                if state == 1:
                    if x_last == 0: # y_last == 0の判定はなくていいの？
                        ...
                        # マーカー表示
                        cv2.drawMarker(img, position=(round((fruit_array_all[0]+50)/100*width), round((fruit_array_all[1])/100*height)),color=(0, 255, 0), markerType=cv2.MARKER_CROSS, markerSize=30, thickness=6, line_type=cv2.LINE_4)
                        # 範囲表示
