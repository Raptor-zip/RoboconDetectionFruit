...
Solenoid_Controller_Node::Solenoid_Controller_Node() : rclcpp::Node("Solenoid_Controller")
{
    ...
    this->KiwiMotionOutSubscription = this->create_subscription<std_msgs::msg::ByteMultiArray>("KiwiMotion_SolenoidOut", rclcpp::QoS(10), std::bind(&Solenoid_Controller_Node::KiwiotionOut_Callback, this, std::placeholders::_1));
...
void Solenoid_Controller_Node::KiwiotionOut_Callback(const std_msgs::msg::ByteMultiArray::SharedPtr md_msg)
{
    if (md_msg->data[8] == 1)
    {
    ...
    for (int i = 0; i < 8; i++)
    {
        this->KiwiMotion_msg.bin[i] = md_msg->data[i];
    }
}
...
void Solenoid_Controller_Node::timer_callback(void)
{
    if (this->motion_flag == 4)
    {
        this->Solenoid_Controll_msg = this->KiwiMotion_msg;
    }
    else if (this->motion_flag == 3)
    {
