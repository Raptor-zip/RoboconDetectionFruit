...
{
    // RCLCPP_INFO(this->get_logger(), "nyuuryokukakunin");
...
void Upper_Controller_Node::ImageRecognition_Callback(const std_msgs::msg::Int16MultiArray::SharedPtr recognition_msg)
{
    ...
    {
        this->upper_msg.M = 194 * this->button_state >> 4 * gONOFF;
        this->up_flag = 0;
        // Sleep(2000)
    }
    if (recognition_msg->data[3] == 1) // grape
    {
        this->upper_msg.M = 118 * this->button_state >> 4 * gONOFF;
        this->up_flag = 0;
        // Sleep(2000)
    }
    if (recognition_msg->data[3] == 2) // mix
    {
        this->upper_msg.M = 95 * this->button_state >> 4 * gONOFF;
        this->up_flag = 0;
        // Sleep(2000)
    }
}
...
void Upper_Controller_Node::timer_callback(void)
{
    ...
    // RCLCPP_INFO(this->get_logger(), "button value %d  ", this->button_state >> 4 & 0b001);
