...
void Upper_Controller_Node::Joy_Callback(const sensor_msgs::msg::Joy::SharedPtr joy_msg)
{
    ...
    // this->x_button = joy_msg->buttons[0];  // share

    if (joy_msg->buttons[0] == 1) // L1
    {
...
void Upper_Controller_Node::ImageRecognition_Callback(const std_msgs::msg::Int16MultiArray::SharedPtr recognition_msg)
{
    ...
    // RCLCPP_INFO(this->get_logger(), "button value %d  ", this->button_state >> 0);
    // RCLCPP_INFO(this->get_logger(), "button value %d  ", this->x_button);
    if (recognition_msg->data[0] == 1 && (double)(end_collection - start_collection) / CLOCKS_PER_SEC > 0.3 && last_lock == 0) //  this->button_state >> 0 == 1
    {
        // RCLCPP_INFO(this->get_logger(), "上下変更可能");
        if (recognition_msg->data[3] == 0) // blueberry
        {
            this->upper_msg.M = 204;
            this->up_flag = 0;
    ...
    last_lock = recognition_msg->data[4];
}
