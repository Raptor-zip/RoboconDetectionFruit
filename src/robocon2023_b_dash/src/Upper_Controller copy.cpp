...
// もし４秒間フルーツを認識しなければ、２５５にする。

int gONOFF = 1; //0:OFF 1:ON

clock_t start_collection, end_collection; //自動回収の開始時刻と終了時刻を格納する変数
clock_t start_detect_finish, end_detect_finish; // 最後にフルーツを認識した時刻
...
void Upper_Controller_Node::ImageRecognition_Callback(const std_msgs::msg::Int16MultiArray::SharedPtr recognition_msg)
{
    ...
    if (this->button_state >> 4 == 1 && recognition_msg->data[0] == 1)
    {
        if ((double)(end_collection - start_collection) / CLOCKS_PER_SEC > 0.3){ // 暴走防止 大体*10した値
            ...
            if (abs(recognition_msg->data[1]) < 20 && recognition_msg->data[2] < 20)
            {
                this->upper_msg.M -= 20;
...
{
    end_detect_finish = clock();
    if ((double)(end_detect_finish - start_detect_finish) / CLOCKS_PER_SEC > 0.4){ // 暴走防止 大体*10した値
        this->upper_msg.M = 250;
    }
