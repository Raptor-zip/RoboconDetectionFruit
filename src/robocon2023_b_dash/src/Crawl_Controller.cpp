...
Crawl_Controller_Node::Crawl_Controller_Node() : rclcpp::Node("Crawl_Controller")
{
    ...
    this->KiwiMotionOutSubscription = this->create_subscription<std_msgs::msg::ByteMultiArray>("KiwiMotion_CrawlOut", rclcpp::QoS(10), std::bind(&Crawl_Controller_Node::KiwiotionOut_Callback, this, std::placeholders::_1));
...
void Crawl_Controller_Node::KiwiotionOut_Callback(const std_msgs::msg::ByteMultiArray::SharedPtr md_msg)
{
    if (md_msg->data[32] == 1)
    {
    ...
    for (int i = 0; i < 8; i++)
    {
        ...
        this->KiwiMotion_msg[2].bin[i] = md_msg->data[i + 16];
        this->KiwiMotion_msg[3].bin[i] = md_msg->data[i + 24];
    }
}
...
void Crawl_Controller_Node::timer_callback(void)
{
    if (this->motion_flag == 4)
    {
        this->Crawl_Controll_msg[0] = this->KiwiMotion_msg[0];
        this->Crawl_Controll_msg[1] = this->KiwiMotion_msg[1];
        this->Crawl_Controll_msg[2] = this->KiwiMotion_msg[2];
        this->Crawl_Controll_msg[3] = this->KiwiMotion_msg[3];
    }
    else if (this->motion_flag == 3)
    {
    ...
    // RCLCPP_INFO(this->get_logger(), " 1,2,3,4   %f,%f,%f,%f  ",
    //             this->Crawl_Controll_msg[0].M, this->Crawl_Controll_msg[1].M,
    //             this->Crawl_Controll_msg[2].M, this->Crawl_Controll_msg[3].M);
