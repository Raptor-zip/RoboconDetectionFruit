...
WallUpMotion_Node::WallUpMotion_Node() : rclcpp::Node("WallUp_Motion")
{
    ...
    this->used_publiser = this->create_publisher<std_msgs::msg::ByteMultiArray>("WallMotion_UseOut", rclcpp::QoS(10));
    this->used_subscription = this->create_subscription<std_msgs::msg::ByteMultiArray>("MotionMasterOut", rclcpp::QoS(10), std::bind(&WallUpMotion_Node::MotionMaster_Code_Callback, this, std::placeholders::_1));
...
void WallUpMotion_Node::Joy_Callback(const sensor_msgs::msg::Joy::SharedPtr joy_msg)
{
    ...
    this->l1 = joy_msg->buttons[4];
    this->r1 = joy_msg->buttons[5];
}

void WallUpMotion_Node::MotionMaster_Code_Callback(const std_msgs::msg::ByteMultiArray::SharedPtr use_msg)
{
...
void WallUpMotion_Node::ToggleClawlCmd(uint8_t crawl_num, uint8_t mode)
{
    if (mode == FAST_MODE)
    {
        // fast
...
void WallUpMotion_Node::InitPos_Clawl(uint8_t crawl_num, uint8_t mode)
{
...
void WallUpMotion_Node::SetClawlIncPos(uint8_t crawl_num, uint8_t mode, float value)
{
    this->crawl_msg[crawl_num].type[0] = 23;
    this->crawl_msg[crawl_num].type[1] = 180;
    this->crawl_msg[crawl_num].ID = crawl_num;
...
void WallUpMotion_Node::SetClawlAbsPos(uint8_t crawl_num, uint8_t mode, float value)
{
    if (mode == FAST_MODE)
    {
        this->crawl_msg[crawl_num].type[0] = 23;
...
float WallUpMotion_Node::GetClawlNowM(uint8_t crawl_num)
{
    return this->crawl_msg[crawl_num].M;
}

float WallUpMotion_Node::GetClawlZEROPos(uint8_t crawl_num)
{
    if (crawl_num == 0)
        return CRAWL1_ZERO;
    if (crawl_num == 1)
...
void WallUpMotion_Node::DriveSpeedControl(uint8_t drive_num, float speed)
{
    this->drive_msg[drive_num].cmd = 4;
    this->drive_msg[drive_num].M = speed;
}

void WallUpMotion_Node::DriveDutyControl(uint8_t drive_num, float duty)
{
    this->drive_msg[drive_num].cmd = 0;
    this->drive_msg[drive_num].M = duty;
}

void WallUpMotion_Node::BeltSpeedControl(uint8_t belt_num, float speed)
{
    this->belt_msg[belt_num].cmd = 4;
    this->belt_msg[belt_num].M = speed;
}

void WallUpMotion_Node::BeltDutyControl(uint8_t belt_num, float duty)
{
...
void WallUpMotion_Node::ResetOldTIme(double now_time, double *old_time)
{
    *old_time = now_time;
}
...
bool WallUpMotion_Node::L1OnCheck(double dt)
{
    double now_time = WallUpMotion_Node::now().seconds();
    static double old_time = 0;
    static uint8_t on_flag = 0;
...
bool WallUpMotion_Node::R1OnCheck(double dt)
{
    double now_time = WallUpMotion_Node::now().seconds();
    static double old_time = 0;
    static uint8_t on_flag = 0;
...
{
    if ((this->motion_master_code == 0) || (this->motion_master_code == 3))
    {
        ...
        if (this->motion_start_flag == 1)
        {
            ...
            case 0:
                break;
            case 1: // front staundup
                if (this->R1OnCheck(0.01))
                {
                    motion_do_count = 0;
                    motion_count++;
                ...
                break;
            case 2: // front holizontal and back standup
                if (this->R1OnCheck(0.01))
                {
                    motion_do_count = 0;
                    motion_count++;
        ...
        if (motion_do_count == 0)
        {
            ...
            case 1: // front standup
                this->SetClawlIncPos(0, FAST_MODE, +50);
                this->SetClawlIncPos(3, FAST_MODE, -50);
                motion_do_count = 1;
                break;

            case 2: // front holizontal and back standup
                this->SetClawlIncPos(0, FAST_MODE, -50);
                this->SetClawlIncPos(1, SLOW_MODE, +60);
                this->SetClawlIncPos(2, SLOW_MODE, -60);
                this->SetClawlIncPos(3, FAST_MODE, +50);
                ...
                break;
            case 3: // back holizontal too
                this->SetClawlIncPos(1, FAST_MODE, -60);
                this->SetClawlIncPos(2, FAST_MODE, +60);
