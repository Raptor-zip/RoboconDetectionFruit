...
# model
# model = YOLO("C:/Users/MS_yu/OneDrive/ドキュメント/vscord/yolov5/best.pt")
...
while True:
    ...
    for r in results:
        ...
        if show == True:
            ...
            # 中央左の線
            cv2.line(img, pt1=(int(width / 2 - width * width_threshold / 2), 0), pt2=(int(width / 2 - width * width_threshold / 2),height), color=(0,255,255), thickness=2)
        ...
        if show == True:
            # FPS表示
            cv2.putText(img, str(f'{fps:.0f}')+"FPS", [30,110], font, fontScale, (0,0,0), int(thickness + 10))
            cv2.putText(img, str(f'{fps:.0f}')+"FPS", [30,110], font, fontScale, color, thickness)

        cv2.putText(img, str(len(boxes))+"length", [30,140], font, fontScale, color, thickness)
        ...
        for box in boxes:
            ...
            if conf > 70:
                if show == True:
                    ...
                    cv2.drawMarker(img, (int((x1+x2)/2), int((y1+y2)/2)), (0, 0, 0), markerType=cv2.MARKER_STAR, markerSize=30,thickness=10)
                    cv2.drawMarker(img, (int((x1+x2)/2), int((y1+y2)/2)), (0, 255, 0), markerType=cv2.MARKER_STAR, markerSize=30,thickness=4)
                    cls = int(box.cls[0])
                    org = [x1 - 10, y1 - 10]
        ...
        if center != 0:
            if center < (width / 2 - (width * (width_threshold / 2))):
                if show == True:
                    cv2.putText(img, "Move to Left", [30,50], font, fontScale*1.2, (0,0,0), thickness + 10)
                    cv2.putText(img, "Move to Left", [30,50], font, fontScale*1.2, (0,255,255), thickness)
                else:
                    print("Left")

            if  (width / 2 - (width * (width_threshold / 2))) < center < (width * 0.5 + (width * (width_threshold / 2))):
                if show == True:
                    cv2.putText(img, "Center", [30,50], font, fontScale*1.2, (0,0,0), thickness + 10)
                    cv2.putText(img, "Center", [30,50], font, fontScale*1.2, (0,255,0), thickness)
                else:
                    print("Center")

            if center > (width / 2 + (width * (width_threshold / 2))):
                if show == True:
                    cv2.putText(img, "Move to Right", [30,50], font, fontScale*1.2, (0,0,0), thickness + 10)
                    cv2.putText(img, "Move to Right", [30,50], font, fontScale*1.2, (255,255,0), thickness)
                else:
                    print("Right")
